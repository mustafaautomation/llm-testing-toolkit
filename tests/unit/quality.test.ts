...
describe('QualityEvaluator', () => {
  ...
  describe('format validation', () => {
    it('validates JSON format', () => {
      ...
      const result = evaluator.evaluate('{"name": "John", "age": 30}', 'Return a JSON object');
      expect(result.formatScore).toBe(1);
    });

    it('fails invalid JSON', () => {
      const evaluator = new QualityEvaluator({ expectedFormat: 'json' });
      const result = evaluator.evaluate('This is not JSON at all', 'Return a JSON object');
  ...
  describe('completeness', () => {
    ...
    it('scores partially for missing sections', () => {
      ...
      const result = evaluator.evaluate('Introduction: Hello. Conclusion: Done.', 'Write a guide');
