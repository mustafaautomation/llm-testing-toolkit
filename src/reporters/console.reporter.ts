...
export class ConsoleReporter {
  ...
  private reportTest(test: TestCaseResult): void {
    const icon = test.error ? `${COLORS.yellow}!` : test.passed ? `${COLORS.green}\u2713` : `${COLORS.red}\u2717`;
    ...
    for (const evaluation of test.evaluations) {
      ...
      console.log(`    ${evalIcon} [${evaluation.evaluator}] ${scoreStr} - ${evaluation.details}${COLORS.reset}`);
    }

    if (test.latencyMs > 0) {
      console.log(`    ${COLORS.dim}Latency: ${test.latencyMs}ms | Tokens: ${test.tokens.input}in/${test.tokens.output}out${COLORS.reset}`);
    }
  }

  private reportSummary(result: TestRunResult): void {
    const { summary } = result;
    const passRateColor = summary.passRate >= 0.8 ? COLORS.green : summary.passRate >= 0.5 ? COLORS.yellow : COLORS.red;
    ...
    console.log(`  ${passRateColor}Pass Rate: ${(summary.passRate * 100).toFixed(1)}%${COLORS.reset}`);

    if (summary.avgLatencyMs > 0) {
      console.log(`  Avg Latency: ${summary.avgLatencyMs}ms`);
      console.log(`  Total Tokens: ${summary.totalTokens.input}in / ${summary.totalTokens.output}out`);
    }
