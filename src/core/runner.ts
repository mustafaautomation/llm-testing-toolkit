...
export class TestRunner {
  ...
  async run(suiteFilter?: string): Promise<TestRunResult> {
    ...
    if (suites.length === 0) {
      throw new Error(
        suiteFilter ? `Suite "${suiteFilter}" not found` : 'No test suites configured',
      );
    }
  ...
  private async runSuite(suite: TestSuite): Promise<SuiteResult> {
    ...
    for (let i = 0; i < suite.tests.length; i += concurrency) {
      ...
      const batchResults = await Promise.all(batch.map((test) => this.runTestCase(test, suite)));
        ...
        case 'hallucination': {
          if (!test.context) {
            return {
              ...
              passed: false,
              score: 0,
              details: 'No context/source docs provided',
            };
          }
